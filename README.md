...
### 🎯 חזון ומטרות
הפרויקט נולד מתוך צורך אמיתי בשוק: זוגות צעירים ומפיקי אירועים נאלצים להשתמש במספר רב של כלים נפרדים (Excel, יומן, רשימות אנשי קשר, אתרי ספקים) כדי לנהל אירוע בודד. EasyEvents נועדה לשנות את המצב:
*   **One Stop Shop:** ריכוז כל כלי הניהול תחת קורת גג אחת, מסונכרנת ומודרנית.
*   **חווית משתמש (UX) יוקרתית:** ממשק נקי, מזמין ואינטואיטיבי שמוריד את מפלס הלחץ הכרוך בהפקת אירוע.
*   **חיסכון כספי:** כלים חכמים לניהול תקציב והשוואת ספקים המונעים חריגות והוצאות מיותרות.
*   **זמינות:** מערכת מבוססת Web הנגישה מכל מכשיר (מחשב, טאבלט, נייד).

### 🔄 זרימת עבודה (User Flow)
תהליך העבודה במערכת תוכנן בקפידה כדי ללוות את המשתמש צעד אחר צעד:
1.  **הרשמה חכמה:** יצירת פרופיל משתמש והגדרת סוג האירוע, תאריך יעד ותקציב משוער.
2.  **Dashboard ניהולי:** מסך ראשי המציג תמונת מצב עדכנית – כמה ימים נותרו, ניצול תקציב, ומשימות דחופות להיום.
3.  **איתור ספקים:** מנוע חיפוש מתקדם לאיתור ספקים לפי קטגוריות (צלמים, אולמות, קייטרינג), כולל יכולת סינון לפי מחיר ודירוג.
4.  **ניהול הפקה:** שימוש בצ'ק-ליסט אינטראקטיבי, מעקב אחרי הצעות מחיר, וסגירת חוזים דיגיטלית.
5.  **ביום האירוע:** גישה מהירה לכל אנשי הקשר הרלוונטיים ולוחות הזמנים ישירות מהנייד.
